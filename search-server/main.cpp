...
private:
    ...
    template<typename Predicate>
    bool Boolean(const int id,const Predicate predicate) const {
        int rt = documents_.at(id).rating; //вычленяем рейтинг 
        DocumentStatus st = documents_.at(id).status; //вычленяем статус
        
        //загружаем в предикат ID  STATUS  RATING
        if (predicate(id, st, rt)) {
            return true;
        } else { return false; }
    }
    ...
    vector<Document> FindAllDocuments(const Query& query, Predicate predicate) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                bool t = Boolean(document_id, predicate);
